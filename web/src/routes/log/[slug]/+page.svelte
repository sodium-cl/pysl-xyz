...
<div class="mt-[5vh] min-h-[90vh] flex flex-col justify-top items-center px-20">
    <div class="flex flex-col md:flex-row justify-top items-center p-10">
        <img
            class="h-[10vh] md:h-[20vh] px-5"
            src={data.post.coverImageURL}
            alt="cover image of {data.post.title}"
        />
    ...
    <div class="flex flex-col justify-between border-2 border-black p-4">
        <span class="my-2 text-sm font-medium"
            >{date(data.post.publishedDate)}</span
        >
        {#each data.post.body as postblock}
            <PortableText value={postblock} />
        {/each}
